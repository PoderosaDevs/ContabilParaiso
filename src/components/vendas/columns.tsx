    ...
    render: (v: Venda) => {
      ...
      return (
        <div className="flex flex-col items-center justify-center gap-1.5 w-full min-w-[160px]">
          ...
          <div
            className={`flex items-center justify-between w-full max-w-[150px] px-2 py-1 rounded border transition-colors ${isPagoCorretamente ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}
          >
            <span
              className={`text-[10px] font-bold flex items-center gap-1 ${isPagoCorretamente ? "text-emerald-600" : "text-amber-600"}`}
            >
              <ArrowDownToLine className="w-3 h-3" /> Recebido
            </span>
            <span
              className={`text-xs font-bold ${isPagoCorretamente ? "text-emerald-700" : "text-amber-700"}`}
            >
